...
function extractSkills(text = "") {
    ...
    return uniqueStrings(
        COMMON_TECH_KEYWORDS.filter((skill) =>
...
function inferRoleTrack({ title, jobDescription }) {
    ...
    if (source.includes("frontend") || source.includes("react") || source.includes("ui")) {
        return "frontend";
    }

    if (source.includes("backend") || source.includes("node") || source.includes("api")) {
        return "backend";
    }

    if (source.includes("full stack") || source.includes("fullstack") || source.includes("mern")) {
        return "fullstack";
    }

    if (source.includes("ai") || source.includes("llm") || source.includes("prompt") || source.includes("machine learning")) {
        return "ai";
    }
...
function inferExperienceLevel(resume = "", title = "") {
    const text = normalizeText(`${resume} ${title}`);
...
function buildRoleQuestions(roleTrack) {
    if (roleTrack === "frontend") {
        return [
            "How do you optimize performance in a React application?",
            "How do you structure reusable components in a large frontend project?",
            "How do you manage state and API data in React?",
            "How would you debug a slow or broken UI in production?"
        ];
    }

    if (roleTrack === "backend") {
        return [
            "How would you design a secure authentication flow in Node.js?",
            "How do you structure REST APIs in Express.js?",
            "How do you handle errors and validation in backend services?",
            "How would you improve database performance for a slow API?"
        ];
    }

    if (roleTrack === "fullstack") {
        return [
            "Explain how your frontend communicates with your backend.",
            "How would you design authentication in a full-stack application?",
            "How do you handle validation on both client and server?",
            "How would you debug an issue where the frontend works but API data is not saving?"
        ];
    }

    if (roleTrack === "ai") {
        return [
            "How would you design an AI feature from prompt to production?",
            "How do you reduce hallucinations in an AI assistant?",
            "How would you evaluate the quality of AI-generated answers?",
            "How do you handle user data safely in an AI product?"
        ];
    }

    return [
        "Tell me about one technical project you are proud of.",
        "How do you debug a difficult technical issue?",
        "How do you learn a new technology for a project?",
        "How do you balance code quality and speed?"
    ];
}
...
function buildQuestionPlan({
    ...
    behavioral.slice(0, 3).forEach((question, index) => {
        plan.push({
            type: "behavioral",
    ...
    return uniqueStrings(plan.map((item) => item.question)).map((question) => {
        return plan.find((item) => item.question === question);
    });
}

function getAskedQuestionsFromHistory(history = "") {
...
function findNextQuestion(plan, history = "") {
    const normalizedHistory = normalizeText(history);

    for (const item of plan) {
        const normalizedQuestion = normalizeText(item.question);

        if (!normalizedHistory.includes(normalizedQuestion)) {
            return item;
        }
    }
...
function countCandidateAnswers(history = "") {
...
function buildFallbackReply({
    ...
    const feedback =
        nextQuestion.type === "technical"
            ? "Good. Try to include a specific example, the technical approach, and the final result."
            : nextQuestion.type === "behavioral"
                ? "Good start. Structure it with situation, action, and result."
                : "Thanks. Keep your answer clear and role-focused.";

    return `${feedback} Next question: ${nextQuestion.question}`;
}
